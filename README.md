...
## Credits
Our dataset extends the [Pie-Perf dataset](https://github.com/madaan/pie-perf?tab=readme-ov-file), which provides a rich collection of paired efficient and inefficient algorithmic implementations. While we retained their original paired snippet representations and problem descriptions, we expanded upon the dataset by profiling execution on hardware to measure and append our newly extracted `energy`, `time`, and `memory` metrics, alongside rigorous LLM-generated taxonomy classifications.
